...
eval_clustering_OOS <- function(
  ...
  if(!latMirrored){
    ### Settind dist matrices 
    D1 = as.dist(D_matrix)
    D2= as.dist( (1-as.numeric(vet_alpha[1]))*D_matrix + as.numeric(vet_alpha[1])*list_normalized_dist$geo_Dist)
    D3= as.dist( (1-as.numeric(vet_alpha[2]))*D_matrix + as.numeric(vet_alpha[2])*list_normalized_dist$geo_Dist)
    D4= as.dist( (1-as.numeric(vet_alpha[3]))*D_matrix + as.numeric(vet_alpha[3])*list_normalized_dist$geo_Dist)
  }else{
    ### Settind dist matrices 
    D1 = as.dist(D_matrix)
    D2 = as.dist( (1-as.numeric(vet_alpha[1]))*D_matrix + as.numeric(vet_alpha[1])*list_normalized_dist$geo_Dist_mirrored)
    D3 = as.dist( (1-as.numeric(vet_alpha[2]))*D_matrix + as.numeric(vet_alpha[2])*list_normalized_dist$geo_Dist_mirrored)
    D4 = as.dist( (1-as.numeric(vet_alpha[3]))*D_matrix + as.numeric(vet_alpha[3])*list_normalized_dist$geo_Dist_mirrored)
  }
