...
gen_plots_uncommonLegend <- function(gridBase,clustMemLim,df_GeoAbio,
  ...
  list_summary_clustRegion = lapply(seq_along(clustRegion), function(i){
    ...
    if(verticalSlices){
        plt = plt + geom_vline(xintercept = c(-60,-40,-5,5,35),col='deepskyblue3')
    }
  ...
  clustRegion_facetd <- ggpubr::ggarrange(
    plotlist = list_summary_clustRegion,#legend.grob = legend_unique,
    ncol = 4,nrow=2)
